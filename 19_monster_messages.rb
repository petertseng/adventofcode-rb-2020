...
rules, messages, *bad = sections
raise "extraneous sections #{bad}" unless bad.empty?

raw_rules = rules.lines.to_h { |line|
...
messages = messages.lines(chomp: true).map(&:freeze).freeze
